...
const LandingPage = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (key.toLowerCase() === expectedKey.toLowerCase()) {
        ...
        // Reset progress if typed key doesn't match
  ...
  const triggerEasterEgg = () => {
    ...
    // Auto reset easter egg banner after 6s
    setTimeout(() => {
  ...
  const handleAddTicketSubmit = (e) => {
    ...
    // Trigger automated simulation of ticket resolution to show "real-time status tracking"
    runInteractiveDemoSimulation(createdTicket.id);
  };

  // Status cycling simulation
  const runInteractiveDemoSimulation = (ticketId) => {
    setIsDemoRunning(true);
    setDemoStep("assigning");

    // After 2.5 seconds, set status to In Progress and assign to a team member
    setTimeout(() => {
      setTickets((prev) =>
        prev.map((t) =>
          t.id === ticketId
    ...
    // After 5 seconds, set status to Resolved
    setTimeout(() => {
      setTickets((prev) =>
        prev.map((t) =>
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden font-poppins transition-colors duration-500 bg-slate-900 text-slate-100 dark:bg-slate-950 light:bg-slate-50 light:text-slate-900">
      ...
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-32 px-6 sm:px-8 lg:px-12 flex flex-col items-center justify-center text-center">
        ...
        <div className="max-w-4xl mx-auto space-y-6 md:space-y-8">
          ...
          {/* Subtle Tagline */}
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold tracking-wide bg-orange-500/10 text-orange-400 dark:text-orange-300 border border-orange-500/20 shadow-sm animate-pulse">
            <Ticket className="w-3.5 h-3.5" />
            <span>Honest, Fast Support Desk</span>
          </div>

          {/* Value Prop Headline */}
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight leading-[1.1] text-slate-100 light:text-slate-900">
          ...
          <p className="text-base sm:text-lg md:text-xl font-normal text-slate-400 light:text-slate-600 max-w-2xl mx-auto leading-relaxed">
      ...
      <section className="py-12 md:py-20 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
        ...
        {/* Mock Browser Frame */}
        <div className="relative bg-slate-800 dark:bg-slate-900 light:bg-white rounded-2xl shadow-2xl border border-slate-700/60 dark:border-slate-800/80 light:border-slate-200/80 overflow-hidden transition-all duration-300 hover:shadow-[0_20px_50px_rgba(0,0,0,0.3)] hover:scale-[1.005] group">
          
          {/* Browser Header Bar */}
          <div className="flex items-center justify-between px-4 py-3 bg-slate-850 dark:bg-slate-950 light:bg-slate-100 border-b border-slate-700/60 dark:border-slate-800/80 light:border-slate-200">
            {/* Window controls */}
            ...
            {/* Fake Address Bar */}
            <div className="w-[60%] max-w-sm px-3 py-1 text-xs text-center rounded bg-slate-800/80 dark:bg-slate-900/80 light:bg-slate-200/85 text-slate-500 truncate">
              https://app.helpsync.io/dashboard
            </div>
            
            {/* Live Indicator */}
            <div className="flex items-center space-x-1.5 bg-orange-500/15 border border-orange-500/20 text-orange-400 px-2 py-0.5 rounded text-[10px] font-semibold tracking-wider uppercase animate-pulse">
          ...
          <div className="p-4 sm:p-6 lg:p-8">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              <div>
                <h3 className="text-xl font-bold text-slate-100 light:text-slate-800">Support Queue</h3>
            ...
            {isDemoRunning && (
              <div className="mb-4 p-3 rounded-lg bg-orange-500/10 border border-orange-500/20 flex items-center space-x-3 text-xs text-orange-400 animate-pulse">
            ...
            <div className="space-y-3">
              {tickets.map((t) => (
                <div
                  key={t.id}
                  className="bg-slate-850 dark:bg-slate-900/60 light:bg-slate-50 border border-slate-750 dark:border-slate-800/80 light:border-slate-200 rounded-xl p-4 transition-all duration-300 hover:translate-x-1 hover:border-orange-500/40 hover:shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4"
                >
                  <div className="flex-1 space-y-1.5">
                    ...
                    <h4 className="text-sm sm:text-base font-bold text-slate-100 light:text-slate-800">{t.title}</h4>
                    <p className="text-xs sm:text-sm text-slate-400 light:text-slate-600 line-clamp-2 leading-relaxed">{t.description}</p>
                  </div>
                  
                  <div className="flex items-center justify-between md:justify-end gap-4 border-t border-slate-850 md:border-t-0 pt-3 md:pt-0">
                    <div className="flex items-center space-x-2 text-xs text-slate-400 light:text-slate-600">
                      <div className="w-6 h-6 rounded-full bg-slate-700 flex items-center justify-center text-[10px] font-bold text-white shadow-sm ring-1 ring-white/10">
      ...
      {/* Value Pillars Section (Honesty check - no fake numbers) */}
      <section className="py-12 md:py-20 px-6 sm:px-8 lg:px-12 max-w-6xl mx-auto">
        <div className="grid md:grid-cols-3 gap-8">
          <div className="bg-slate-850 dark:bg-slate-900/40 light:bg-white p-6 rounded-2xl border border-slate-750 dark:border-slate-800/80 light:border-slate-200/80 shadow-md">
            <div className="w-12 h-12 bg-orange-500/10 text-orange-500 rounded-xl flex items-center justify-center mb-4">
              <Zap className="w-6 h-6" />
            </div>
            <h3 className="text-lg font-bold text-slate-100 light:text-slate-800 mb-2">Real-Time Sync</h3>
          ...
          <div className="bg-slate-850 dark:bg-slate-900/40 light:bg-white p-6 rounded-2xl border border-slate-750 dark:border-slate-800/80 light:border-slate-200/80 shadow-md">
            ...
            <h3 className="text-lg font-bold text-slate-100 light:text-slate-800 mb-2">Zero Bloat Dashboard</h3>
            <p className="text-sm text-slate-400 light:text-slate-600 leading-relaxed">
          ...
          <div className="bg-slate-850 dark:bg-slate-900/40 light:bg-white p-6 rounded-2xl border border-slate-750 dark:border-slate-800/80 light:border-slate-200/80 shadow-md">
            ...
            <h3 className="text-lg font-bold text-slate-100 light:text-slate-800 mb-2">Honest Team Workspaces</h3>
            <p className="text-sm text-slate-400 light:text-slate-600 leading-relaxed">
      ...
      {/* Mini Create Ticket Modal (Inside Browser Demo Mock) */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 light:bg-white rounded-2xl border border-slate-800 light:border-slate-200 w-full max-w-md p-6 shadow-2xl transform scale-100 transition-all">
            <div className="flex items-center justify-between mb-4 border-b border-slate-800 light:border-slate-200 pb-3">
              <h3 className="text-lg font-bold text-slate-100 light:text-slate-850 flex items-center gap-1.5">
                <Ticket className="w-4 h-4 text-orange-500" />
                <span>Simulate New Ticket</span>
              </h3>
              <button
                onClick={() => setIsModalOpen(false)}
                className="text-slate-500 hover:text-slate-300 hover:bg-slate-800 light:hover:bg-slate-100 p-1.5 rounded-lg transition-colors cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleAddTicketSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-400 light:text-slate-600 uppercase tracking-wider mb-1.5">
                  Ticket Subject
                </label>
                <input
                  type="text"
                  ...
                  className="w-full bg-slate-850 light:bg-slate-50 border border-slate-750 light:border-slate-250 text-slate-100 light:text-slate-900 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500/30 transition-all placeholder-slate-550"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-850 light:bg-slate-50 border border-slate-750 light:border-slate-250 text-slate-100 light:text-slate-900 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500/30 transition-all placeholder-slate-550 resize-none"
                />
              </div>
